...
export async function validateOrder(orderId) {
  ...
  // Use SECURITY DEFINER RPC to bypass RLS issues on UPDATE
  const { data: rpcData, error: rpcError } = await supabase.rpc("validate_order", {
    order_id: orderId,
  });

  if (rpcError) {
    console.error("[Order] Validate RPC error:", rpcError.message);
